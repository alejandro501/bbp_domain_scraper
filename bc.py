...
# Extract changelog URL from the engagement HTML content
...
# Fetch changelog and extract scope data
...
def main(config, targets_file='targets.txt', domains_file='domains.txt', invalid_urls_file='invalid_urls.txt'):
    ...
    with open(targets_file, 'w') as targets_file_handle:
...
# Process targets file to separate into wildcards.txt, domains.txt, and invalid_urls.txt
def process_targets_file(targets_file, domains_file, invalid_urls_file):
    ...
    if invalid_urls:
        with open(invalid_urls_file, 'w') as invalid_file:
            invalid_file.write('\n'.join(invalid_urls) + '\n')
        print(f"Invalid URLs saved to {invalid_urls_file} ({len(invalid_urls)} found)")

    if valid_urls:
        with open(domains_file, 'w') as valid_file:
            valid_file.write('\n'.join(valid_urls) + '\n')
