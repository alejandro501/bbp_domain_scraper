...
def main(config, targets_file='targets.txt', domains_file='domains.txt', invalid_urls_file='invalid_urls.txt'):
    ...
    with open(targets_file, 'w') as targets_file_handle:  # Updated to use the passed targets_file parameter
    ...
    process_targets_file(targets_file, domains_file, invalid_urls_file) 
    base_url = "https://bugcrowd.com/engagements.json?category=bug_bounty&page={}&sort_by=promoted&sort_direction=desc"
...
def process_targets_file(targets_file, domains_file, invalid_urls_file):
    # (Keep the existing function code but update file writing sections)
    
    wildcards = []
    invalid_urls = []
    valid_urls = []

    # Reading the targets from the main targets_file...
    ...
    if wildcards:
        with open(domains_file, 'w') as wildcard_file:  # Use domains_file instead of hard-coded name
            wildcard_file.write('\n'.join(wildcards) + '\n')
        print(f"Wildcards saved to {domains_file} ({len(wildcards)} found)")

    if invalid_urls:
        with open(invalid_urls_file, 'w') as invalid_file:  # Use invalid_urls_file instead of hard-coded name
            invalid_file.write('\n'.join(invalid_urls) + '\n')
        print(f"Invalid URLs saved to {invalid_urls_file} ({len(invalid_urls)} found)")

    if valid_urls:
        with open('valid_urls.txt', 'w') as valid_file:  # Keep this hard-coded, since it's not part of args
            valid_file.write('\n'.join(valid_urls) + '\n')
